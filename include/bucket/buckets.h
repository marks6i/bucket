...
{ 
	...
	public:
		...
		using bucket_type = triplet<index_type,
		                           index_type,
		                           value_container>;
		...
		struct accessor {
			...
			[[nodiscard]] static constexpr index_type& high(bucket_type& t) noexcept                    { return t.second; }
			[[nodiscard]] static constexpr const index_type& high(const bucket_type& t) noexcept        { return t.second; }
			[[nodiscard]] static constexpr value_container& values(bucket_type& t) noexcept             { return t.third;  }
			[[nodiscard]] static constexpr const_value_container& values(const bucket_type& t) noexcept { return t.third;  }
		};
	...
	public:
		...
		[[nodiscard]] constexpr std::size_t size() const noexcept { return buckets_.size(); } /**< Number of buckets in buckets collection */
		[[nodiscard]] constexpr bool empty() const noexcept { return buckets_.empty(); } /**< Boolean indicating if the buckets collection is empty */
		[[nodiscard]] constexpr index_type low() const noexcept { return low_; } /**< Lower bounds of a constrained buckets collection. Deprecated in favor of lower_bound.  */
		[[nodiscard]] constexpr index_type high() const noexcept { return high_; } /**< Upper bounds of a constrained buckets collection. Deprecated in favor of upper_bound. */
		[[nodiscard]] constexpr bool constrained() const noexcept { return constrained_; }/**< Return true if buckets collection is constrained. Deprecated in favor of is_constrained. */
	...
	public:
		...
		[[nodiscard]] int spread(const buckets<Indices, Values, Traits, OtherContainerTraits>& bucket_)
		{
			int added_to_bucket = 0;
		 ...
		 */
		template <class OtherContainerTraits>
		[[nodiscard]] int cover(const buckets<Indices, Values, Traits, OtherContainerTraits>& bucket_)
		{
			int added_to_bucket = 0;
