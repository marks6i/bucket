...
`bucket_map` is a template class that provides a map-like container for storing ordered ranges with associated values. Each bucket represents a non-overlapping range on an ordered axis, with gaps allowed if there are no values in the given range.
...
// Get the lower bound of a constrained map (throws if unconstrained)
[[nodiscard]] index_type low() const;
[[nodiscard]] index_type lower_bound() const;

// Get the upper bound of a constrained map (throws if unconstrained)
...
- Throws `std::runtime_error` when accessing bounds of an unconstrained map